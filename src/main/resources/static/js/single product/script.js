...
// Responsive nav dropdowns
	...
	function selectSize(size) {
		// Đặt logic xử lý khi chọn size ở đây (ví dụ: đổi màu chữ, thêm hiệu ứng, gửi dữ liệu, v.v.)
		// Ở đây, chúng ta chỉ đơn giản thêm class "selected" cho size được chọn
		const sizeOptions = document.querySelectorAll('.size-option');
		sizeOptions.forEach(option => option.classList.remove('selected'));
		const selectedSize = document.querySelector(`.size-option:contains(${size})`);
		selectedSize.classList.add('selected');
	}

	// Hàm hỗ trợ tìm element theo nội dung
	// Chú ý: Hàm này không được hỗ trợ mặc định trong JavaScript, nên phải thêm nó vào
	// hoặc sử dụng thư viện như jQuery.
	document.querySelector(':contains');

	function contains(selector, text) {
		var elements = document.querySelectorAll(selector);
		return Array.from(elements).filter(element => RegExp(text, 'i').test(element.textContent));
	}
